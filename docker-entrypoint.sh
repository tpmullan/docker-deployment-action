...
case $INPUT_DEPLOYMENT_MODE in
  ...
  *)
    ...
    DEPLOYMENT_COMMAND="docker $DEPLOYMENT_COMMAND_OPTIONS compose -f $STACK_FILE"
  ;;
esac
...
if ! [ -z "$INPUT_DOCKER_PRUNE" ] && [ $INPUT_DOCKER_PRUNE = 'true' ] ; then
  yes | docker --host "ssh://$INPUT_REMOTE_DOCKER_HOST:$INPUT_REMOTE_DOCKER_PORT" system prune -a 2>&1
fi
